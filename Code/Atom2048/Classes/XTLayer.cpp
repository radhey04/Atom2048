...
: Layer()
{
    this->_xtSwipeThreshold = 5;
    this->_xtSwipeTime = 500;
    this->_xtTapThreshold = 10;
    this->_xtTapTime = 250;
    this->_xtLongTapTreshold = 10;
    this->_xtLongTapTime = 500;
    this->_xtDoubleTapTime = 400;
...
void XTLayer::onTouchesBegan(const std::vector<Touch*>& _touches, cocos2d::Event* event)
{
    ...
    this->_touchHasEnded = false;
    this->_longtouchDone = false;
    ...
    this->runAction(Sequence::create(DelayTime::create(this->_xtLongTapTime/1000), CallFunc::create([this](){XTLayer::checkXTLongTapGesture(); }), NULL));
    // Passthrough
...
void XTLayer::checkXTLongTapGesture()
{
    if(!this->_touchHasEnded && (!this->_touchHasMoved || (fabs(this->xtActualPoint.x - this->xtGestureStartPoint.x) < 50 &&  fabs(this->xtActualPoint.y - this->xtGestureStartPoint.y) < 50)))
    {
        this->_longtouchDone = true;
        this->xtLongTapGesture(this->xtGestureEndPoint);
    }
}
...
void XTLayer::onTouchesMoved(const std::vector<Touch *>& _touches, cocos2d::Event* event)
{
    ...
    xtDragGesture(this->xtActualPoint - fingerOne->getDelta(), fingerOne->getDelta());
...
{
    if(!this->_longtouchDone)
    {
        this->stopAllActions();
        this->_touchHasEnded = true;
        ...
        else if (this->_dragStarted)// No movement, tap detected
        {
            this->xtDragEnded(location);
            this->_dragStarted = false;
        }
        else
        {
...
void XTLayer::tapHandler(float dt)
{
    ...
    else if (this->_xtNumberOfTaps >= 2) // double and more tap
    {
        this->_xtNumberOfTaps = 0;
